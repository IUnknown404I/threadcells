...
A same-build service restart keeps the provider-side control connection compatible. After a promoted build changes privileged orchestration code, an old connection is fenced before it can create an effect. If the active identity is temporarily unavailable during restart, the operation is rejected without an effect and retried after the service returns. A proven mismatch asks the provider to reinitialize that control connection through its normal capacity-limited input path, preserves the conversation, and retries the already durable logical turn rather than creating a replacement workflow.
