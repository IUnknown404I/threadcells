...
La vista previa es de solo lectura. La admisión de la ejecución requiere el desbloqueo de operador existente y de corta duración y el modal de confirmación de acción permanente existente; no hay contraseña de Full Cleanup ni secreto almacenado por el cliente. La admisión crea un identificador opaco de operación vinculado al `plan_id` exacto de 64 caracteres y no incluye ninguna ruta arbitraria. Una vez admitida, la operación conserva progreso duradero y un informe terminal tras desconexiones del navegador, el vencimiento del desbloqueo y reinicios del plano de control. Repetir la misma operación la observa en vez de iniciar otra pasada destructiva; un plan distinto requiere una nueva autorización.

Cada candidato de Full Cleanup basado en una ruta lo ejecuta el root helper de ámbito reducido, activado por socket, después de consumir una capacidad de servidor de un solo uso, reconstruir el plan exacto, demostrar la barrera de inactividad y verificar que el plano de control aún mantiene todos los límites de admisión. La credencial sin procesar del operador nunca se envía al helper. El helper mueve cada candidato a una cuarentena exclusiva de root en el mismo sistema de archivos, bloquea el árbol de directorios capturado frente a mutaciones del usuario de runtime y después elimina únicamente las identidades verificadas mediante descriptores de directorio. Persiste el informe terminal acotado antes de responder; la reconciliación tras reinicio y durante el sondeo conserva un helper exacto aún vivo o informa un resultado fallido/indeterminado veraz sin repetir la pasada destructiva. Una identidad modificada se conserva y se incluye en el informe; los recursos de ciclo de vida ajenos al sistema de archivos siguen pasando por sus ejecutores transaccionales canónicos.
