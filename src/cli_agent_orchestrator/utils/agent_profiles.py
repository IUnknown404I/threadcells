...
# Packaged profiles are product authority shipped with ThreadCells. A same-named
# legacy/provider file must not silently shadow their behavior. Custom profiles
# use the existing discovery precedence.
CANONICAL_BUILTIN_PROFILE_IDS = frozenset(
    {
        "architect_sol_high",
        "code_supervisor",
...
def _list_legacy_agent_profiles() -> List[Dict]:
...
def list_agent_profiles() -> List[Dict]:
    """List canonical registry profiles after bootstrap, else legacy discovery."""
    ...
    if not registry_is_initialized():
        return _list_legacy_agent_profiles()
    return [
        {
            ...
            "execution_mode": item["document"]["execution_mode"],
            "owner_authorization_required": item["owner_authorization_required"],
        }
...
def parse_agent_profile_text(resolved_text: str, profile_name: str) -> AgentProfile:
    ...
    if "description" not in meta:
        meta["description"] = ""
    profile = AgentProfile(**meta)
    if profile.mcpServers:
        profile.mcpServers = {
            server_name: canonicalize_cao_mcp_server_config(
                server_name,
                (
                    server_config
...
def _load_legacy_agent_profile(agent_name: str) -> AgentProfile:
    ...
    try:
        if agent_name in CANONICAL_BUILTIN_PROFILE_IDS:
            agent_store = resources.files("cli_agent_orchestrator.agent_store")
...
def load_agent_profile(agent_name: str) -> AgentProfile:
    """Load from canonical SQLite after bootstrap, otherwise legacy files."""
    _validate_agent_name(agent_name)
    from cli_agent_orchestrator.services.control_plane_registry import (
    ...
    if not registry_is_initialized():
        return _load_legacy_agent_profile(agent_name)
    try:
        return resolve_launch(agent_name, fallback_provider="codex").profile
    except RegistryNotFoundError as error:
        raise FileNotFoundError(f"Agent profile not found: {agent_name}") from error
