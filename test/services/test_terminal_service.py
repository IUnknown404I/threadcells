...
def test_privileged_profile_rejects_prompt_token_without_server_grant(tmp_path):
    from cli_agent_orchestrator.services.operations_service import AdmissionDenied

    with pytest.raises(AdmissionDenied) as rejected:
        create_terminal(
            provider="codex",
    ...
    assert rejected.value.reason_code == "OWNER_GRANT_REQUIRED"


def test_existing_session_privileged_add_rejects_without_server_grant(tmp_path):
    from cli_agent_orchestrator.services.operations_service import AdmissionDenied
...
def test_resolved_custom_privileged_profile_cannot_bypass_owner_grant(monkeypatch, tmp_path):
    ...
    resolution = SimpleNamespace(
        profile=AgentProfile(
            name="custom-owner",
            description="Custom privileged profile",
            role="supervisor",
    ...
    monkeypatch.setattr(
        control_plane_registry, "resolve_launch", lambda *args, **kwargs: resolution
    )

    with pytest.raises(AdmissionDenied) as rejected:
        create_terminal(
            provider="codex",
            agent_profile="custom-owner",
            new_session=True,
            working_directory=str(tmp_path),
        )
