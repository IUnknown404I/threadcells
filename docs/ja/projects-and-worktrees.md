...
正確なリビジョンのレビューは、永続化されたレビュー試行とレビュアーの物理チェックアウトの両方を結び付けます。ThreadCells はレビュータスクを配信する前に端末入力をフェンスし、セッション所有のクリーンなレビュアー worktree を検証し、要求された commit へ detached モードで移動して、プロバイダーへの転送時にリビジョンを再確認します。対象を絞った修正でレビュアーを再利用すると、新しい試行が作成され、新しいリビジョンが準備されます。以前の結果は履歴として残り、その修正を承認できません。
