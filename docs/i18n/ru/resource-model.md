...
ThreadCells разделяет ёмкость, потому что работа coding-agent в разное время может нагружать разные части хоста. Ход модели потребляет ёмкость провайдера; назначенный контекст программирования может оставаться активным, когда модель простаивает; сборка может насытить машину после прекращения вывода модели.
