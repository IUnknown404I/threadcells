...
function HousekeepingReport({ report, diskState }: { report: Record<string, any> | null; diskState: string }) {
  ...
  return <div className="space-y-3">
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      <Summary label="Result" value={completedWithIssues ? 'Completed with issues' : 'Completed'} />
      <Summary label="Started" value={started} />
      <Summary label="Completed" value={completed} />
      <Summary label="Duration" value={duration} />
      <Summary label="Reclaimed total" value={bytes(report.freed_bytes)} detail={report.observed_disk_free_delta === undefined ? undefined : `${bytes(report.observed_disk_free_delta)} observed free-space change`} />
      <Summary label="Resulting disk health" value={diskState} />
    </div>
    {report.full_cleanup && <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
      <Summary label="Planned reclaim" value={bytes(report.reclaimable_bytes)} detail={`${Number(report.planned_candidates || 0)} resources`} />
      <Summary label="Disk before / after" value={`${bytes(report.disk_before)} / ${bytes(report.disk_after)}`} />
      <Summary label="Releases removed" value={String(Number(report.releases_removed || 0))} detail={`Rollback: ${report.rollback_available === false ? 'none' : report.rollback_available === true ? 'available' : 'not proven'}`} />
      <Summary label="Active release" value={report.active_release || 'Authority not proven'} />
      <Summary label="Worktrees retired" value={String(Number(report.worktrees_retired || 0))} />
      <Summary label="Caches / builds / temp removed" value={String(fullCounts)} />
      <Summary label="Logs removed" value={String(Number(report.logs_deleted || 0))} />
      <Summary label="Protected / skipped" value={String(skipped)} />
    </div>}
    <div className="grid gap-3 lg:grid-cols-2">
      <div className="rounded-xl border border-gray-700/60 bg-gray-800/60 p-4"><h3 className="text-sm font-semibold text-gray-200">Reclaimed by class</h3>{classes.length ? <dl className="mt-3 grid grid-cols-2 gap-2">{classes.map(([label, value]) => <div key={label} className="rounded-lg bg-gray-900/50 p-2"><dt className="text-xs text-gray-400">{label.replace(/_/g, ' ')}</dt><dd className="mt-1 text-sm text-gray-200">{bytes(Number(value || 0))}</dd></div>)}</dl> : <p className="mt-3 text-xs text-gray-400">No class reclaimed bytes in this run.</p>}</div>
      <div className="rounded-xl border border-gray-700/60 bg-gray-800/60 p-4"><h3 className="text-sm font-semibold text-gray-200">Protection and errors</h3><p className="mt-3 text-sm text-gray-300">{skipped} protected or skipped item{skipped === 1 ? '' : 's'}</p>{protectedResources.length ? <ul className="mt-2 max-h-40 list-disc space-y-1 overflow-auto pl-5 text-xs text-amber-200">{protectedResources.map((item: any, index: number) => <li key={`protected-${index}`}>{item.canonical_identity ? `${item.canonical_identity}: ` : ''}{item.reason || 'Protected'}{item.category ? ` · ${String(item.category).replace(/_/g, ' ')}` : ''}{item.bytes === undefined ? '' : ` · ${bytes(Number(item.bytes || 0))}`}</li>)}</ul> : null}{executionSkips.length ? <ul className="mt-2 max-h-40 list-disc space-y-1 overflow-auto pl-5 text-xs text-amber-200">{executionSkips.map((item: any, index: number) => <li key={`skip-${index}`}>{item.candidate ? `${item.candidate}: ` : ''}{item.reason_code || 'Skipped during execution'}</li>)}</ul> : null}{report.execution_failures?.length ? <ul className="mt-2 list-disc space-y-1 pl-5 text-xs text-red-300">{report.execution_failures.map((item: any, index: number) => <li key={`failure-${index}`}>{item.reason_code || 'Execution failure'}</li>)}</ul> : <p className="mt-2 text-xs text-gray-400">No execution failures recorded.</p>}<HousekeepingWarnings warnings={report.warnings || []} className="mt-2" /></div>
    </div>
    <Artifact value={report} label="Raw report"/>
  </div>
}
...
function HousekeepingSettingsPage() {
  ...
  const buildPlan = async () => {
    ...
    try {
      ...
      if (!controller.signal.aborted && mountedRef.current) {
        setPlan(null)
        setError(planningError(reason, 'ThreadCells could not build the Housekeeping plan. No files were deleted. Try again.'))
      }
    } finally {
  ...
  const buildFullPlan = async () => {
    ...
    try {
      ...
      if (!controller.signal.aborted && mountedRef.current) {
        setFullPlan(null)
        setFullError(planningError(reason, 'ThreadCells could not build the Full Cleanup preview. No files were deleted. Try again.'))
      }
    } finally {
      finishPlanning(controller)
    }
  ...
  const runFull = async () => {
    ...
    try {
      ...
      if (reason instanceof CaoApiError && reason.reasonCode === 'HOUSEKEEPING_PLAN_CHANGED') {
        setFullPlan(null)
        setFullError('The inspected Full Cleanup plan changed before execution. Build and inspect a fresh preview.')
        ...
        setFullError(reason.message || 'Full Cleanup failed')
      }
      await access.refresh()
  ...
  const executeDisabledReason = planning
    ? 'Wait for the current filesystem inventory to finish before executing a plan.'
    : running
  ...
  const fullDisabledReason = planning
    ? 'Wait for the current filesystem inventory to finish before running Full Cleanup.'
    : fullRunning
      ? 'Full Cleanup is executing under the canonical Housekeeping and lifecycle locks.'
    : !fullPlan
      ? 'Build and inspect a Full Cleanup preview before execution.'
      : !fullPlan.idle_gate.eligible
        ? 'Full Cleanup is blocked until every agent is Ready or Exited and provider and Heavy execution are idle.'
        : fullActionable.length === 0
          ? 'This preview has no proven-safe removable resources.'
          : access.loading
            ? 'Checking operator authorization.'
            : !access.status?.configured
              ? 'Operator changes are not configured.'
              : !access.status.authenticated
                ? 'Unlock operator changes with the existing operator secret.'
                : null
    ...
    <section className="overflow-hidden rounded-xl border border-red-800/70 bg-red-950/20" aria-labelledby="full-cleanup-heading">
      <div className="border-b border-red-900/60 p-4 sm:p-5">
        <div className="flex items-start gap-3">
          <span className="rounded-lg bg-red-900/50 p-2 text-red-300"><Trash2 size={18} aria-hidden="true" /></span>
          <div className="min-w-0">
            <h2 id="full-cleanup-heading" className="text-base font-semibold text-red-100">Delete all system files — Full Cleanup</h2>
            <p className="mt-1 text-xs leading-5 text-red-200/80">Permanently remove every ThreadCells filesystem artifact that can be proven unnecessary while preserving the active system and usable agents.</p>
          </div>
        </div>
        <p className="mt-3 rounded-lg border border-red-800/60 bg-red-950/40 p-3 text-sm font-medium text-red-100">{!fullPlan || fullPlan.release_state.active_only_expected ? 'Only the active local ThreadCells release will remain. Local rollback will be removed.' : `Every proven inactive release will be removed, but ${fullPlan.release_state.protected_non_active_releases} ambiguous release${fullPlan.release_state.protected_non_active_releases === 1 ? '' : 's'} will remain protected. Active-only retention and rollback removal are not yet proven.`}</p>
      </div>
      <div className="space-y-4 p-4 sm:p-5">
        <div className="flex flex-col gap-3 sm:flex-row">
          <button aria-busy={planning === 'full'} disabled={Boolean(planning) || fullRunning || running} className="min-h-11 rounded-lg bg-gray-700 px-4 text-sm disabled:opacity-50" onClick={() => void buildFullPlan()}>{planning === 'full' ? <span className="inline-flex items-center gap-2"><Loader2 size={15} className="animate-spin" aria-hidden="true"/>Building preview…</span> : 'Build Full Cleanup preview'}</button>
          <button aria-describedby={fullDisabledReason ? 'full-cleanup-disabled-reason' : undefined} disabled={Boolean(fullDisabledReason)} className="min-h-11 rounded-lg bg-red-600 px-4 text-sm font-semibold text-white disabled:opacity-40" onClick={() => setFullConfirm(true)}>{fullRunning ? 'Running Full Cleanup…' : 'Delete all proven-safe system files'}</button>
        </div>
        {planning === 'full' && <p role="status" className="text-xs leading-5 text-gray-300">Scanning releases, worktrees, caches and protected resources. This may take a while.</p>}
