...
def _legacy_profile_discovery_mode(monkeypatch):
    """This module covers the pre-bootstrap Markdown compatibility path."""
    monkeypatch.setattr(
...
class TestLoadAgentProfile:
    ...
    def test_parse_agent_profile_retains_codex_config(self):
        """Codex profile overrides survive frontmatter parsing for the provider."""
...
""",
    ...
    def test_parse_agent_profile_pins_stale_cao_mcp_launcher(self):
        profile = parse_agent_profile_text(
            """---
name: stale-profile
description: Test profile
mcpServers:
  cao-mcp-server:
    command: uvx
    args: ["--from", "git+https://example.com/cao.git@main", "cao-mcp-server"]
---
System prompt
""",
            "stale-profile",
        ...
        assert profile.mcpServers == {"cao-mcp-server": {"command": "threadcells-mcp-server"}}
