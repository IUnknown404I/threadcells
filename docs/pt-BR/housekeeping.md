...
A prévia é somente leitura. A admissão da execução exige o desbloqueio de operador existente e de curta duração e o modal existente de confirmação de ação permanente; não há senha de Full Cleanup nem segredo armazenado pelo cliente. A admissão cria uma ID opaca de operação vinculada ao `plan_id` exato de 64 caracteres e não carrega nenhum caminho arbitrário. Depois de admitida, a operação mantém progresso durável e um relatório terminal apesar de desconexões do navegador, expiração do desbloqueio e reinícios do plano de controle. Repetir a mesma operação apenas a observa, em vez de iniciar outra passagem destrutiva; um plano diferente exige nova autorização.

Cada candidato do Full Cleanup baseado em caminho é executado pelo root helper de escopo restrito, ativado por socket, depois que ele consome uma capacidade de servidor de uso único, reconstrói o plano exato, comprova a barreira de ociosidade e verifica que o plano de controle ainda mantém todos os limites de admissão. A credencial bruta do operador nunca é enviada ao helper. O helper move cada candidato para uma quarentena exclusiva de root no mesmo sistema de arquivos, bloqueia a árvore de diretórios capturada contra mutações pelo usuário de runtime e então exclui somente as identidades verificadas por meio de descritores de diretório. Ele persiste o relatório terminal limitado antes de responder; a reconciliação no reinício e na consulta preserva um helper exato ainda ativo ou informa um resultado verdadeiro de falha/indeterminação sem repetir a passagem destrutiva. Uma identidade alterada é preservada e informada; recursos de ciclo de vida que não pertencem ao sistema de arquivos continuam passando por seus executores transacionais canônicos.
