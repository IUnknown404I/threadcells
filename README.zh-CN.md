[English](README.md) · [Русский](README.ru.md) · **简体中文** · [Español](README.es.md) · [Português (Brasil)](README.pt-BR.md) · [Deutsch](README.de.md) · [日本語](README.ja.md)
...
请从[什么是 ThreadCells？](docs/OVERVIEW.md)、[快速设置](QUICK_SETUP.md)和[您的第一个项目与智能体](docs/FIRST_AGENT.md)开始。完整公开指南涵盖[安装](docs/INSTALLATION.md)、[核心概念](docs/CONCEPTS.md)、[Telegram 通知](docs/TELEGRAM_NOTIFICATIONS.md)、[远程访问](docs/REMOTE_ACCESS.md)、[安全](SECURITY.md)和[运维](docs/OPERATIONS.md)。产品内的 `/docs` 阅读器提供同一套经过 allowlist 筛选并打包的文档语料。
...
`0.3.0-alpha.1` 技术预览版支持单台 Ubuntu/Debian Linux 主机，默认采用 loopback 访问，并以 Codex 为首要配置。原生智能体可以执行高权限命令；worktree 不是安全沙箱。评估前请阅读[当前限制](docs/LIMITATIONS.md)。

公开 OCI 包 `ghcr.io/iunknown404i/threadcells-release-bundle` 携带经过验证的发布归档和证据。它是分发制品，不是 Docker 镜像，也不是受支持的容器部署方式；请参阅[发布流程](docs/RELEASE_PROCESS.md)。
...
**我能远程访问 UI 吗？** 可以，同时仍让 ThreadCells 仅监听 loopback。偶尔访问时使用 SSH 隧道；在主机所有者明确批准访问边界后，也可以使用带身份验证的 Caddy/Authelia HTTPS 反向代理。切勿把原始 ThreadCells 端口暴露到公共互联网；请参阅[远程访问](docs/REMOTE_ACCESS.md)。
...
ThreadCells 是 AWS Labs CLI Agent Orchestrator 的独立、非官方下游项目。Amazon Web Services 不赞助也不认可本项目。原始上游项目采用 Apache License 2.0；请参阅 [NOTICE](NOTICE)、[项目来源](docs/PROVENANCE.md)和[相较上游的变更](docs/CHANGES_FROM_UPSTREAM.md)。
