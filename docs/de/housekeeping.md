...
source_sha256: sha256:48e0a1d83b0677dafa4e242714cae1a45cd17580827e361d8dfa25b4034356ee
---
# Housekeeping
...
Die Vorschau ist schreibgeschützt. Die Zulassung zur Ausführung erfordert die bestehende kurzlebige Operator-Entsperrung und das bestehende Bestätigungsmodal für dauerhafte Aktionen; es gibt kein Full-Cleanup-Passwort und kein clientseitig gespeichertes Secret. Die Zulassung erzeugt eine undurchsichtige Operations-ID, die an die exakte 64 Zeichen lange `plan_id` gebunden ist, und enthält keinen beliebigen Pfad. Nach der Zulassung bleiben Fortschritt und Abschlussbericht über Browser-Trennungen, den Ablauf der Entsperrung und Neustarts der Control Plane hinweg dauerhaft erhalten. Eine Wiederholung derselben Operation beobachtet sie, statt einen zweiten destruktiven Durchlauf zu starten; ein anderer Plan erfordert eine neue Autorisierung.

Jeder pfadnamensbasierte Full-Cleanup-Kandidat wird durch den eng begrenzten, socket-aktivierten Root-Helper ausgeführt, nachdem dieser eine einmal verwendbare Server-Capability verbraucht, den exakten Plan neu erstellt, das Leerlauf-Gate nachgewiesen und geprüft hat, dass die Control Plane weiterhin alle Admission-Fences hält. Das rohe Operator-Credential wird nie an den Helper gesendet. Der Helper verschiebt jeden Kandidaten in eine Root-exklusive Quarantäne auf demselben Dateisystem, sperrt den erfassten Verzeichnisbaum gegen Mutationen durch den Runtime-Benutzer und löscht anschließend ausschließlich die verifizierten Identitäten über Verzeichnisdeskriptoren. Er speichert den begrenzten Abschlussbericht vor seiner Antwort; Neustart- und Polling-Abgleich erhalten einen noch lebenden exakten Helper oder melden ein wahrheitsgemäßes fehlgeschlagenes/unbestimmtes Ergebnis, ohne den destruktiven Durchlauf zu wiederholen. Eine geänderte Identität wird aufbewahrt und gemeldet; nicht dateisystemgebundene Lebenszyklusressourcen laufen weiterhin über ihre kanonischen transaktionalen Executor-Pfade.
