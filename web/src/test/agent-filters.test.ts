...
const agents = [
  { activity: 'ready', lifecycle: 'running' as const, workflow_state: 'active' as const, status: 'idle' },
  { activity: 'ready', lifecycle: 'running' as const, workflow_state: 'owner_gate' as const, status: 'idle' },
  { activity: 'queued', lifecycle: 'running' as const, workflow_state: 'waiting' as const, status: 'idle' },
  { activity: 'exited', lifecycle: 'exited' as const, workflow_state: 'cancelled' as const, status: 'idle' },
  { activity: 'ready', lifecycle: 'running' as const, workflow_state: 'completed' as const, status: 'completed' },
  { activity: 'processing', lifecycle: 'running' as const, workflow_state: 'active' as const, status: 'processing' },
]
...
describe('canonical agent filters', () => {
  it('keeps every Home count equal to its preconfigured Agents status predicate', () => {
    ...
    expect(agents.filter(agent => matchesHomeAgentFilter(agent, 'active'))).toHaveLength(5)
