...
function ExpandedSessionAgents({
  ...
  return <div id={`home-session-detail-${session.id}`} className="space-y-2 border-t border-gray-700/30 px-3 pb-4 pt-3 sm:px-4">
    ...
    {feed.loading && feed.items.length === 0 ? <p className="text-xs text-gray-400">Loading agent details…</p> : <div data-testid="session-agent-container" className={view === 'grid' ? 'grid grid-cols-1 gap-2 lg:grid-cols-2' : 'space-y-2'}>{feed.items.map(agent => {
  ...
  return <div className="space-y-6">
    ...
    {sessionFeed.loading && sessionFeed.items.length === 0 ? <div className="rounded-xl border border-gray-700/50 bg-gray-800/60 p-8 text-center text-sm text-gray-400">Loading session summaries…</div> : sessionFeed.items.length === 0 ? <div className="rounded-xl border border-gray-700/50 bg-gray-800/60 p-8 text-center"><Bot size={32} className="mx-auto mb-3 text-gray-400"/><p className="text-sm text-gray-400">No matching sessions.</p></div> : <div className="space-y-3">{sessionFeed.items.map(session => {
      ...
      return <div key={session.id} data-testid={`home-session-${session.id}`} className={`overflow-hidden rounded-xl border transition-colors ${expanded ? 'border-emerald-700/50 bg-emerald-900/30' : 'border-gray-700/50 bg-gray-800/60'}`}>
        <div data-testid={`session-header-${session.id}`} className="flex min-w-0 items-center gap-2 p-3 sm:gap-3 sm:px-4"><div role="button" tabIndex={0} aria-expanded={expanded} aria-controls={`home-session-detail-${session.id}`} aria-label={`${expanded ? 'Collapse' : 'Expand'} ${displayName}`} onClick={() => toggleSession(session.id)} onKeyDown={event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); toggleSession(session.id) } }} className="flex min-w-0 flex-1 cursor-pointer items-center gap-2 rounded-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400 sm:gap-3"><Users size={14} className="text-emerald-400"/><span className="min-w-0 truncate font-mono text-sm text-gray-200" title={displayName}>{displayName}</span><span className="shrink-0 text-xs text-gray-400">{session.agent_count} agent{session.agent_count === 1 ? '' : 's'}</span>{session.project_name && <span className="min-w-0 max-w-[14rem] truncate rounded-full bg-gray-700/50 px-2 py-0.5 text-xs text-gray-300">Project: {session.project_name}</span>}</div><div className="flex shrink-0 items-center gap-1"><button type="button" onClick={event => { event.stopPropagation(); setPendingDeleteSession(session) }} className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-lg text-gray-400 hover:bg-gray-800 hover:text-red-400" title="Delete session" aria-label={`Delete ${displayName}`}><Trash2 size={14}/></button><button type="button" onClick={() => toggleSession(session.id)} className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-lg text-gray-400 hover:bg-gray-800 hover:text-gray-300" aria-expanded={expanded} aria-controls={`home-session-detail-${session.id}`} aria-label={`${expanded ? 'Collapse' : 'Expand'} ${displayName} using chevron`}>{expanded ? <ChevronDown size={14}/> : <ChevronRight size={14}/>}</button></div></div>
