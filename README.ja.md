[English](README.md) · [Русский](README.ru.md) · [简体中文](README.zh-CN.md) · [Español](README.es.md) · [Português (Brasil)](README.pt-BR.md) · [Deutsch](README.de.md) · **日本語**
...
[ThreadCells とは？](docs/OVERVIEW.md)、[クイックセットアップ](QUICK_SETUP.md)、[最初のプロジェクトとエージェント](docs/FIRST_AGENT.md)から始めてください。公開ガイド全体では、[インストール](docs/INSTALLATION.md)、[基本概念](docs/CONCEPTS.md)、[Telegram 通知](docs/TELEGRAM_NOTIFICATIONS.md)、[リモートアクセス](docs/REMOTE_ACCESS.md)、[セキュリティ](SECURITY.md)、[運用](docs/OPERATIONS.md)を扱います。プロダクト内の `/docs` リーダーは、同じパッケージ済みの許可リスト付きドキュメントコーパスを提供します。
...
`0.2.0-alpha.1` 技術プレビューは、単一の Ubuntu/Debian Linux ホスト、ループバック優先のアクセス、Codex を中心としたセットアップをサポートします。ネイティブエージェントは強力なコマンドを実行できます。worktree はセキュリティサンドボックスではありません。評価前に[制限事項](docs/LIMITATIONS.md)を確認してください。
...
**UI にリモートアクセスできますか？** はい。ThreadCells をループバック専用のままにできます。ときどきのアクセスには SSH トンネルを使うか、アクセス境界についてホスト所有者が明示的に承認した後で、認証済み Caddy/Authelia HTTPS プロキシを使います。生の ThreadCells ポートを公衆インターネットへ公開してはいけません。[リモートアクセス](docs/REMOTE_ACCESS.md)を参照してください。
...
質問、初期アイデア、コミュニティのセットアップには [GitHub Discussions](https://github.com/IUnknown404I/threadcells/discussions) を使ってください。確認済みで実行可能な公開プロジェクト作業には、整理された [GitHub Issues](https://github.com/IUnknown404I/threadcells/issues) バックログを使います。迅速な経路は [CONTRIBUTING.md](CONTRIBUTING.md)、適格性とトリアージは[正規の Issue ポリシー](docs/ISSUES.md)、脆弱性の非公開報告は [SECURITY.md](SECURITY.md)を読んでください。
...
ThreadCells は AWS Labs CLI Agent Orchestrator の独立した非公式 downstream です。Amazon Web Services の後援または支持を受けていません。オリジナルの上流プロジェクトは Apache License 2.0 でライセンスされています。[NOTICE](NOTICE)、[来歴](docs/PROVENANCE.md)、[上流からの変更](docs/CHANGES_FROM_UPSTREAM.md)を参照してください。
