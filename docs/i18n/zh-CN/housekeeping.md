...
部署还可为向后兼容的 CI 缓存命名精确的 ThreadCells 所有缓存前缀。这些条目仍受限于运行时所有根目录下的直接子目录，并要求保留期已过，以及相同的活动进程和执行时身份检查。未列出的前缀（包括含糊的发布候选工件）仍受保护。
...
发布版本和候选版本清理共享规范暂存锁，并要求可信引用元数据。活动和回滚运行时始终受保护。请参阅[升级](UPGRADING.md)。
