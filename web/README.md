...
A single-page dashboard for managing ThreadCells sessions, agents, flows, and settings from the browser.
...
1. **`api.ts`** — HTTP client wrapping all `threadcells-server` REST endpoints (sessions, terminals, agents, flows, settings). Handles timeouts and error responses.
...
|------|----------|---------|-----|
| **Development** | `npm run dev` (Vite dev server with hot-reload) | `threadcells-server` | `http://localhost:5173` |
| **Production** | `npm run build` (static files emitted into `src/cli_agent_orchestrator/web_ui/`) | `threadcells-server` serves the bundled UI from the installed package | `http://localhost:9889` |

In development mode, Vite proxies API requests (`/sessions`, `/terminals`, `/agents`, `/flows`, `/settings`, `/health`) to `threadcells-server` on port 9889 (configured in `vite.config.ts`).
...
| ThreadCells managed (compatibility path) | `~/.aws/cli-agent-orchestrator/agent-context` |
...
web/
├── src/
│   ├── api.ts              # REST + WebSocket client for threadcells-server
...
# Start dev server (requires threadcells-server running on :9889)
npm run dev
