...
function AgentViewControls({ value, onChange }: { value: 'list' | 'grid'; onChange: (value: 'list' | 'grid') => void }) {
  ...
  return <div className="hidden shrink-0 items-center gap-1 sm:inline-flex" role="group" aria-label="Agent layout">{controls.map(({ value: controlValue, label, icon: Icon }) => <button key={controlValue} type="button" aria-label={label} aria-pressed={value === controlValue} onClick={() => onChange(controlValue)} className={`inline-flex h-9 w-9 items-center justify-center rounded-md transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400 ${value === controlValue ? 'bg-emerald-600 text-white' : 'text-gray-400 hover:bg-gray-800 hover:text-gray-200'}`} title={controlValue === 'list' ? 'List' : 'Grid'}><Icon size={15} aria-hidden="true" /></button>)}</div>
}
...
function ExpandedSessionAgents({
  ...
  return <div id={`home-session-detail-${session.id}`} className="space-y-2 border-t border-gray-700/30 px-3 pb-4 pt-3 sm:px-4">
    ...
    {feed.loading && feed.items.length === 0 ? <p className="text-xs text-gray-400">Loading agent details…</p> : <div data-testid="session-agent-container" className={view === 'grid' ? 'space-y-2 sm:grid sm:grid-cols-1 sm:gap-2 sm:space-y-0 lg:grid-cols-2' : 'space-y-2'}>{feed.items.map(agent => {
      ...
      return <div key={agent.id} data-testid={`agent-detail-card-${agent.id}`} className="flex flex-col space-y-2 rounded-lg border border-gray-700/30 bg-gray-900/50 p-3">
        <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
          <div className="flex min-w-0 flex-wrap items-center gap-x-3 gap-y-1"><TermIcon size={14} className="shrink-0 text-gray-400"/><span className="min-w-0 max-w-full truncate text-sm font-medium text-gray-200" title={agent.agent_profile || 'default'}>{agent.agent_profile || 'default'}</span><span className="min-w-0 max-w-full truncate font-mono text-xs text-gray-400" title={agent.id}>{agent.id}</span><StatusBadge status={badge} workflowReason={agent.workflow_reason}/><span className="max-w-full truncate text-[10px] text-gray-400" title={agent.provider}>{agent.provider}</span>{agent.project_name && <span className="max-w-full truncate text-[10px] text-gray-400">{agent.project_name}</span>}</div>
