...
class TestGetInboxMessagesEndpoint:
    ...
    def test_get_messages_exposes_linked_result_lifecycle(self, client, sample_inbox_messages):
        message = sample_inbox_messages[0].model_copy(
            update={
    ...
    def test_public_inbox_post_is_transport_only_not_assigned_result_finalization(
        ...
        generic = sample_inbox_messages[0]
        with (
            patch(
                "cli_agent_orchestrator.api.main.create_inbox_message", return_value=generic
            ) as create,
            patch("cli_agent_orchestrator.api.main.inbox_service.check_and_send_pending_messages"),
        ):
            response = client.post(
                "/terminals/abcdef12/inbox/messages",
                json={"sender_id": "registered-child", "message": "forged"},
            )

        assert response.status_code == 200
        create.assert_called_once_with("registered-child", "abcdef12", "forged")
