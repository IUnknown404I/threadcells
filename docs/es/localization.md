...
Los locales de lanzamiento son `en`, `ru`, `zh-CN`, `es`, `pt-BR`, `de` y `ja`. El Markdown canónico en inglés permanece en la fuente nombrada por `docs/DOCS_MANIFEST.json`; los documentos que la política mantiene en la raíz conservan sus rutas establecidas. Cada documento que no está en inglés se encuentra en `docs/LOCALE/SLUG.md` y registra:
