...
Резервные копии — только инвентарь и никогда не удаляются автоматически. Неизвестные или активные ресурсы остаются защищёнными. Full Cleanup — отдельно подтверждаемое действие оператора, которое выполняется только в простое всех агентов, сохраняет полномочия Ready для продолжения и намеренно удаляет каждый доказанно неактивный локальный релиз, после чего локальный откат становится недоступен. См. [Housekeeping](HOUSEKEEPING.md).
