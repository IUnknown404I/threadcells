...
В Settings → General каждый браузер может показывать абсолютные серверные метки времени в определённом им часовом поясе или в выбранной вручную зоне IANA. Настройка хранится в браузере и не является серверной. Преобразуются только метки с явно заданным UTC или числовым смещением. Длительности, календарные даты без времени и неоднозначные метки времени остаются без изменений.
