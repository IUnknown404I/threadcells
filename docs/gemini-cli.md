...
The Gemini compatibility adapter enables ThreadCells to work with [Gemini CLI](https://github.com/google-gemini/gemini-cli), Google's coding-agent CLI. Gemini CLI runs as an interactive Ink-based TUI (not alternate screen mode) that keeps scrollback history in tmux.
...
- The provider waits up to 240 seconds for initialization (accounts for `-i` prompt processing)
