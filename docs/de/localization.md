...
Release-Gebietsschemata sind `en`, `ru`, `zh-CN`, `es`, `pt-BR`, `de` und `ja`. Kanonisches englisches Markdown bleibt in der von `docs/DOCS_MANIFEST.json` benannten Quelle; durch Richtlinien im Repository-Stamm verankerte Dokumente behalten ihre etablierten Pfade. Jedes nicht englische Dokument befindet sich unter `docs/LOCALE/SLUG.md` und enthält:
