...
import { BookOpen, Boxes, CheckCircle2, Clock3, Database, HardDrive, HeartHandshake, Info, Search, ShieldCheck, Sparkles, Trash2 } from 'lucide-react'
...
function HousekeepingSettingsPage() {
  ...
  const buildFullPlan = async () => {
    setFullError('')
    setFullReport(null)
    try { setFullPlan(await api.getFullCleanupPlan()) }
    catch (reason: any) { setFullPlan(null); setFullError(reason.message || 'Could not build a Full Cleanup preview') }
  }
  const runFull = async () => {
  ...
  const fullDisabledReason = fullRunning
    ...
    <section className="overflow-hidden rounded-xl border border-red-800/70 bg-red-950/20" aria-labelledby="full-cleanup-heading">
      ...
      <div className="space-y-4 p-4 sm:p-5">
        <div className="flex flex-col gap-3 sm:flex-row">
          <button className="min-h-11 rounded-lg bg-gray-700 px-4 text-sm" onClick={() => void buildFullPlan()}>Build Full Cleanup preview</button>
