...
Preview is read-only. Execution admission requires the existing short-lived operator unlock and the existing permanent-action confirmation modal; there is no Full Cleanup password or client-stored secret. Admission creates one opaque operation ID bound to the exact 64-character `plan_id` and carries no arbitrary path. Once admitted, that operation keeps durable progress and a terminal report across browser disconnects, unlock expiry, and control-plane restarts. Repeating the same operation observes it rather than starting another destructive pass; a different plan requires fresh authorization.

Every pathname-based Full Cleanup candidate is executed by the narrow, socket-activated root helper after it consumes a single-use server capability, rebuilds the exact plan, proves the idle gate, and verifies the control plane still holds every admission fence. The raw operator credential is never sent to the helper. The helper moves each candidate into a root-exclusive same-filesystem quarantine, locks the captured directory tree against runtime-user mutation, and then deletes only the verified identities through directory descriptors. It persists the bounded terminal report before replying; restart and polling reconciliation preserve a still-live exact helper or report a truthful failed/indeterminate outcome without replaying the destructive pass. A changed identity is retained and reported, and non-filesystem lifecycle resources continue through their canonical transactional executors.
