...
ThreadCells retains the compatible `CaoPlugin`, `PluginRegistry`, and `cao.plugins` entry-point contract for observer plugins that react to server-side session/terminal lifecycle and message events. They run inside the compatible server process and are notified after an operation.

Observer plugins are not Provider Adapters. They cannot register provider identity/capabilities, start model turns, supply executable provider configuration, or participate in admission/authorization. Provider integrations use the separately trusted `threadcells.provider_adapters.v1` contract described in [Provider adapter authoring](PROVIDER_ADAPTERS.md).
...
**Important:** plugins today are **observers only**. They receive events *after* the corresponding action has already happened, and cannot block, modify, or reject ThreadCells operations. See [Future improvements](#future-improvements) for possible additions.
