...
export function AgentPanel({
  ...
  const handleDeleteSession = async () => {
    ...
    if (!deletePreflight.eligible && !deletePreflight.can_resolve_and_delete && !deletePreflight.can_preserve_protected_workspace) return
    ...
    try {
      ...
      const deleted = deletePreflight.requires_protected_workspace_confirmation
        ? await deleteSession(...deletionArgs, true)
        : await deleteSession(...deletionArgs)
      if (!deleted) {
  ...
  const renderAgentCard = (terminal: AgentSummary, sessionName?: string, grid = false) => {
    ...
    const recoveryCapability = recoveryCapabilities[terminal.id]
    return (
    <div key={terminal.id} data-testid={`agent-detail-card-${terminal.id}`} className="agent-detail-card bg-gray-900/50 border border-gray-700/30 rounded-lg p-3 space-y-2">
      <div className={`flex flex-col justify-between gap-3 ${grid ? '' : 'xl:flex-row xl:items-center'}`}>
        ...
        <div role="group" aria-label={t('agents.actions')} data-testid={`agent-actions-${terminal.id}`} className="agent-action-strip ml-auto grid h-11 max-w-full shrink-0 self-end">
          <button type="button" aria-label={t('interactions.action')} onClick={() => setInteractionTarget({ sessionId: terminal.session_id, sessionName: terminal.session_name, terminalId: terminal.id })} className="agent-action-secondary flex h-11 min-w-0 items-center justify-center gap-2 rounded-lg bg-gray-700 px-0 text-xs font-medium text-white transition-colors hover:bg-gray-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gray-300" title={t('interactions.action')}><History size={14} className="shrink-0"/><span className="agent-action-label">{t('interactions.history')}</span></button>
          <button type="button" aria-label={t('agents.viewInbox')} onClick={() => setInboxTerminal({ id: terminal.id, readOnly: workspaceUnavailable })} className="agent-action-secondary flex h-11 min-w-0 items-center justify-center gap-2 rounded-lg bg-gray-700 px-0 text-xs font-medium text-white transition-colors hover:bg-gray-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gray-300" title={t('agents.viewInbox')}><Mail size={14} className="shrink-0"/><span className="agent-action-label">{t('agents.inbox')}</span></button>
          <button type="button" aria-label={t('agents.viewOutput')} onClick={() => setOutputTerminalId(terminal.id)} className="agent-action-secondary flex h-11 min-w-0 items-center justify-center gap-2 rounded-lg bg-gray-700 px-0 text-xs font-medium text-white transition-colors hover:bg-gray-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gray-300" title={t('agents.viewOutput')}><FileText size={14} className="shrink-0"/><span className="agent-action-label">{t('agents.output')}</span></button>
      ...
      {!workspaceUnavailable && <div className="flex min-w-0 items-start justify-end gap-1">
        <div className="min-w-0 flex-1"><WorkflowRecoveryNotice agent={terminal} capability={recoveryCapability} /></div>
        {recoveryCapability?.eligible === true && <div data-testid={`agent-recovery-action-${terminal.id}`} className="shrink-0">
          <RecoveryTakeoverAction agent={terminal} capability={recoveryCapability} onCompleted={() => { setRecoveryRefreshKey(value => value + 1); activeSessionFeed.reload(); filteredAgentFeed.reload(); sessionFeed.reload() }} iconOnly className="flex h-11 w-11 items-center justify-center rounded-lg bg-indigo-700 text-white transition-colors hover:bg-indigo-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-300"/>
        </div>}
      </div>}
