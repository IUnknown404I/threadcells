...
class TestAssignSenderIdInjection:
    ...
    def managed_relation(self, monkeypatch):
        monkeypatch.setattr(
            "cli_agent_orchestrator.mcp_server.server.register_child_assignment", lambda *_: True
        )
        monkeypatch.setattr(
    ...
    def test_assign_no_suffix_when_injection_disabled(self, mock_create, mock_wait, mock_send):
        """Assigned work leaves admission binding to the internal API path."""
        from cli_agent_orchestrator.mcp_server.server import _assign_impl
        ...
        assert mock_send.call_args[0][2] == "assign"
        assert sent_message.startswith("Analyze the logs")
        assert "[Assigned by terminal" not in sent_message
        assert sent_message.endswith("NO_TG_NOTIFY")
    ...
    def test_assign_suffix_is_appended_not_prepended(self, mock_create, mock_wait, mock_send):
        """The sender ID stays a suffix within the enveloped child payload."""
        from cli_agent_orchestrator.mcp_server.server import _assign_impl
        ...
        assert sent_message.index(original) == 0
        assert sent_message.endswith("NO_TG_NOTIFY")


def test_direct_delegation_uses_the_bound_internal_api_path():
    """MCP direct sends cannot provide a model-selected logical turn ID."""
    from cli_agent_orchestrator.constants import API_BASE_URL
    from cli_agent_orchestrator.mcp_server.server import _send_direct_input
    from cli_agent_orchestrator.models.inbox import OrchestrationType

    with patch("cli_agent_orchestrator.mcp_server.server.requests.post") as post:
        post.return_value.raise_for_status.return_value = None
        _send_direct_input("worker-1", "task", OrchestrationType.ASSIGN, "opaque-binding")

    assert post.call_args.args[0] == f"{API_BASE_URL}/_internal/terminals/worker-1/input"
    assert post.call_args.kwargs["params"]["binding"] == "opaque-binding"
    assert "logical_turn_id" not in post.call_args.kwargs["params"]
