...
const selectedViewport = process.env.WEBSITE_LOCALE_VIEWPORT

if (!selectedViewport) {
  for (const viewport of viewportDefinitions) {
    const child = spawnSync(process.execPath, [process.argv[1]], {
      env: { ...process.env, WEBSITE_LOCALE_VIEWPORT: viewport.name },
      stdio: 'inherit',
    })
    if (child.status !== 0) process.exit(child.status ?? 1)
  }
  console.log(JSON.stringify({ locales: locales.length, viewports: viewportDefinitions.length, routes: locales.length * viewportDefinitions.length * 3, evidenceDir, basePath }))
  process.exit(0)
}

const viewports = viewportDefinitions.filter(viewport => viewport.name === selectedViewport)
assert.equal(viewports.length, 1, `unknown viewport: ${selectedViewport}`)
  ...
  for (const viewport of viewports) {
    for (const { locale, prefix, lang } of locales) {
      ...
      try {
        ...
        await page.screenshot({ path: path.join(evidenceDir, `${locale}-landing-${viewport.name}.png`), animations: 'disabled' })
        routes += 1
        ...
        for (const slug of ['installation', 'housekeeping']) {
          ...
          await page.screenshot({ path: path.join(evidenceDir, `${locale}-${slug}-${viewport.name}.png`), animations: 'disabled' })
          routes += 1
        }
      } finally {
