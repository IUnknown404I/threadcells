  ...
  return <div className="space-y-6">
    ...
    {sessionFeed.loading && sessionFeed.items.length === 0 ? <div className="rounded-xl border border-gray-700/50 bg-gray-800/60 p-8 text-center text-sm text-gray-400">Loading session summaries…</div> : sessionFeed.items.length === 0 ? <div className="rounded-xl border border-gray-700/50 bg-gray-800/60 p-8 text-center"><Bot size={32} className="mx-auto mb-3 text-gray-400"/><p className="text-sm text-gray-400">No matching sessions.</p></div> : <div className="space-y-3">{sessionFeed.items.map(session => {
      ...
      return <div key={session.id} data-testid={`home-session-${session.id}`} className={`overflow-hidden rounded-xl border transition-colors ${expanded ? 'border-emerald-700/50 bg-emerald-900/30' : 'border-gray-700/50 bg-gray-800/60'}`}>
        ...
        <div className="flex min-w-0 items-start gap-2 border-t border-gray-700/30 px-3 py-2 sm:items-center sm:px-4" aria-label="Session status"><SessionStatusSummary session={session} trailing={<AgentViewControls value={agentView} onChange={setAgentView}/>} /></div>
        {expanded && (
    ...
    <ConfirmModal open={!!pendingDeleteSession} title="Delete Session" message="This removes the historical session from ThreadCells. Protected filesystem resources and their cleanup authority remain intact. This action cannot be undone." details={pendingDeleteSession ? [{ label: 'Session', value: sessionDisplayName(pendingDeleteSession.name) }, { label: 'Status', value: pendingDeleteSession.status }] : []} confirmLabel="Delete Session" variant="danger" loading={!!deletingSession} onConfirm={handleDeleteSession} onCancel={() => setPendingDeleteSession(null)}/>
  </div>
}
