...
class ClaudeCodeProvider(BaseProvider):
    ...
    def _ensure_skip_bypass_prompt_setting() -> None:
        """Apply an explicit operator-requested Claude startup preference.
        ...
        ``skipDangerousModePermissionPrompt: true`` is persisted. This helper
        is deliberately not called by initialize or preflight: provider launch
        must not silently mutate another tool's operator configuration.
        """
    ...
    def _handle_startup_prompts(self, timeout: float = 20.0) -> None:
        ...
        1. **Bypass permissions confirmation** (``--dangerously-skip-permissions``)
           ...
           An explicit operator preference can prevent it; this handler safely
           supports an otherwise unconfigured first launch.
