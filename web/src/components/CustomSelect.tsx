...
export function CustomSelect({ value, onChange, options, placeholder, className = '' }: CustomSelectProps) {
  ...
  return (
    <div ref={ref} className={`relative ${className}`}>
      ...
      {open && (
        <div className="absolute z-50 mt-1 w-full max-w-[calc(100vw-1.5rem)] bg-gray-900 border border-gray-700 rounded-lg shadow-xl shadow-black/30 max-h-64 overflow-y-auto">
          {groups.map((group, gi) => (
            <div key={gi}>
              {group.label && (
              ...
              {group.items.map(opt => (
                <button
                  ...
                  className={`w-full min-h-11 text-left px-3 py-2 flex items-center justify-between gap-2 transition-colors ${
                    opt.disabled
