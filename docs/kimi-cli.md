...
The Kimi compatibility adapter enables ThreadCells to work with [Kimi Code CLI](https://kimi.com/code), Moonshot AI's coding-agent CLI. Kimi CLI runs as an interactive TUI using prompt_toolkit.
...
```bash
kimi --yolo --mcp-config '{"server-name": {"command": "cao-mcp-server"}}'
```
