...
def cleanup_old_data():
    ...
    try:
        ...
        with SessionLocal() as db:
            deleted_messages = (
                db.query(InboxModel)
                .filter(InboxModel.created_at < cutoff_date, InboxModel.result_id.is_(None))
                .delete()
            )
            db.commit()
            logger.info(f"Deleted {deleted_messages} old inbox messages from database")

        deleted_results = purge_expired_delegation_results(cutoff_date)
        logger.info(
            "Deleted %s expired delegation results and their closed relations", deleted_results
        )
