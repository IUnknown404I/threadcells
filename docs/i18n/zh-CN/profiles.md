...
内置 `critical_sol_xhigh_owner` 配置文件可在两个 Web 启动流程中选择：创建会话或向现有会话添加代理。两者都会显示例外权限提示，要求明确确认及短时有效的操作员解锁，然后才铸造并消耗一份正常启动能力。Add Agent 会将该能力限定到现有会话及规范继承/项目工作目录。本地 CLI 通过 `--owner-xhigh` 和交互式确认提供相同的权限类别。这些路径均不会创建可复用的 API 绕过，也不会授权其他配置文件、子终端或无关的 Settings 更改。
