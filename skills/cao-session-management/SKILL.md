...
Top-level supervisors may start in the canonical project worktree. When a
delegation omits an explicit working directory, CAO isolates it automatically:
execution owners reuse one linked task branch/worktree and reviewers get a
detached worktree at the exact reviewed commit. This shares Git objects,
performs no clone or dependency installation, and removes only clean verified
worktrees after safe result/lifecycle handling.
