...
export function TelegramSettings() {
  ...
  if (!settings) return <p className="py-8 text-center text-sm text-gray-500">Loading Telegram settings…</p>
  ...
  return <section className="min-w-0 space-y-5" aria-labelledby="telegram-settings-heading">
    ...
    <div className="min-w-0 space-y-5 rounded-xl border border-gray-700/60 bg-gray-800/60 p-4 sm:p-5">
      <label className="flex min-h-11 min-w-0 items-center justify-between gap-4 rounded-lg border border-gray-700 bg-gray-900/60 px-3"><span className="min-w-0"><span className="block text-sm font-medium text-gray-200">Enabled</span><span className="block text-xs leading-5 text-gray-500">Disabling delivery keeps the configured destination and token.</span></span><input aria-label="Enable Telegram notifications" type="checkbox" checked={enabled} onChange={event => setEnabled(event.target.checked)} className="h-5 w-5 shrink-0 accent-emerald-500"/></label>
      <div className="grid min-w-0 gap-4 md:grid-cols-2">
        <label className="min-w-0 text-sm text-gray-300">Chat ID / destination
          ...
          <span className="mt-1 block text-xs leading-5 text-gray-500">The global chat or channel destination used for every Project.</span>
        </label>
        <label className="min-w-0 text-sm text-gray-300">Topic / message-thread ID <span className="text-gray-600">(optional)</span>
          ...
          <span className="mt-1 block text-xs leading-5 text-gray-500">Used as Telegram&apos;s message_thread_id when the destination is a forum topic.</span>
        </label>
      </div>
      <label className="block min-w-0 text-sm text-gray-300">Bot token
        ...
        <span className="mt-1 block text-xs leading-5 text-gray-500">The token is stored in ThreadCells-owned restrictive secret storage. It is never returned by this API or saved in browser storage.</span>
      </label>
