...
class TestSessionNameValidation:
    @pytest.mark.parametrize(
        ...
        [
            ("CAO.UI.Q1", "CAO_UI_Q1"),
            ("release.2026.08", "release_2026_08"),
            ("PLATFORM.OVERVIEW.UI.P1", "PLATFORM_OVERVIEW_UI_P1"),
            (
                "WORKSPACE.UI.P1 — Responsive Final Correction & Implementation Handoff",
                "WORKSPACE_UI_P1 — Responsive Final Correction & Implementation Handoff",
            ),
            (r"release../\\next::candidate", "release_next_candidate"),
            ("..", "_"),
        ],
    )
    def test_normalizes_tmux_structural_characters(self, session_name, expected_name):
        assert validate_session_name(session_name) == expected_name

    def test_trims_only_outer_whitespace(self):
        assert (
            validate_session_name("  CAO — Mobile UI Adaptation  ") == "CAO — Mobile UI Adaptation"
        )

    @pytest.mark.parametrize("session_name", ["", " \t "])
    def test_rejects_empty_name_after_normalization(self, session_name):
        with pytest.raises(ValueError, match="Session name must not be empty"):
            validate_session_name(session_name)

    def test_rejects_nul_as_genuinely_invalid(self):
        with pytest.raises(ValueError, match="must not contain NUL"):
            validate_session_name("release\x00candidate")
