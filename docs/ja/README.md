...
`0.4.2-alpha` 技術プレビューは、単一の Ubuntu/Debian Linux ホスト、ループバック優先のアクセス、Codex を中心としたセットアップをサポートします。ネイティブエージェントは強力なコマンドを実行できます。worktree はセキュリティサンドボックスではありません。評価前に[制限事項](../LIMITATIONS.md)を確認してください。
