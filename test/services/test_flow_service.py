...
""")
    ...
    @patch("cli_agent_orchestrator.services.flow_service.db_update_flow_run_times")
    @patch("cli_agent_orchestrator.services.flow_service.db_get_flow")
    def test_deleted_project_flow_uses_frozen_snapshot(
        self, mock_db_get, _update, _session, create, _send
    ):
        with tempfile.NamedTemporaryFile(mode="w", suffix=".md", delete=False) as f:
            f.write(
                '---\nname: historical\nschedule: "* * * * *"\nagent_profile: developer\n---\nPrompt.'
            ...
            mock_db_get.return_value = Flow(
                ...
                schedule="* * * * *",
                agent_profile="developer",
                project_id="6d7606b5-32ba-4f6b-8b1b-edc4ac0cbd80",
                project_name="Frozen A",
                project_path="/legacy/a",
                project_description="frozen comment",
            )
        create.return_value = MagicMock(id="terminal")
        with (
            patch(
                "cli_agent_orchestrator.services.flow_service.project_service.launch_context",
                side_effect=project_service.ProjectResolutionError("Unknown projectId"),
            ),
            patch(
        ...
        assert create.call_args.kwargs["project_context"] == {
            "id": "6d7606b5-32ba-4f6b-8b1b-edc4ac0cbd80",
            "name": "Frozen A",
            "path": "/legacy/a",
            "description": "frozen comment",
        }
        registry.assert_called_once_with("6d7606b5-32ba-4f6b-8b1b-edc4ac0cbd80")
    ...
    @patch("cli_agent_orchestrator.services.flow_service.db_update_flow_run_times")
    @patch("cli_agent_orchestrator.services.flow_service.db_get_flow")
    def test_registered_flow_project_overrides_its_frozen_snapshot(
        self, mock_db_get, _update, _session, create, _send
    ):
        with tempfile.NamedTemporaryFile(mode="w", suffix=".md", delete=False) as f:
            f.write(
        ...
        assert create.call_args.kwargs["project_context"] == current
