...
Техническая предварительная версия `0.3.0-alpha.2` поддерживает один хост Ubuntu/Debian Linux, доступ через loopback по умолчанию и настройку прежде всего для Codex. Нативные агенты могут выполнять мощные команды; worktree не являются песочницей безопасности. Перед оценкой продукта прочитайте [ограничения](docs/LIMITATIONS.md).
