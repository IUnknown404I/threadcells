...
def validate_session_name(session_name: str) -> str:
    """Normalize a user-facing session name to a tmux-safe internal name."""
    candidate = session_name.strip()
    if not candidate:
        raise ValueError("Session name must not be empty")
    if "\x00" in candidate:
        raise ValueError("Invalid session name: must not contain NUL")

    # tmux uses dot and colon in target syntax; slash and backslash also carry
    # path semantics at CAO boundaries.  Preserve display-friendly text while
    # collapsing each structural run to one safe character.
    normalized_name = re.sub(r"[.:/\\]+", "_", candidate)
