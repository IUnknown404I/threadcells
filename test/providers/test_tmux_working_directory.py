...
class TestTmuxClientWorkingDirectory:
    ...
    def test_get_pane_process_id_requires_one_exact_live_pane(self):
        mock_session = Mock()
        mock_window = Mock()
        mock_pane = Mock()
        self.mock_server.sessions.get.return_value = mock_session
        ...
        mock_result = Mock(stdout=["0 4321"])
        mock_pane.cmd.return_value = mock_result

        client = TmuxClient()

        assert client.get_pane_process_id("test-session", "test-window") == 4321
        mock_pane.cmd.assert_called_once_with("display-message", "-p", "#{pane_dead} #{pane_pid}")

    def test_get_pane_process_id_rejects_ambiguous_or_dead_panes(self):
        mock_session = Mock()
        mock_window = Mock()
        self.mock_server.sessions.get.return_value = mock_session
        mock_session.windows.get.return_value = mock_window
        mock_window.panes = [Mock(), Mock()]
        client = TmuxClient()
        assert client.get_pane_process_id("test-session", "test-window") is None

        mock_pane = Mock()
        mock_pane.cmd.return_value = Mock(stdout=["1 4321"])
        mock_window.panes = [mock_pane]
        assert client.get_pane_process_id("test-session", "test-window") is None
