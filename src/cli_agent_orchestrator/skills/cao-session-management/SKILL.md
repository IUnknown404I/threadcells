...
description: Interact with ThreadCells through compatible CAO commands — launch multi-agent sessions,
...
Session status is provider state, not semantic workflow completion. A completed
provider turn may receive one persisted continuation after a child result or
while the workflow remains OPEN; do not send a duplicate task to compensate.
Use normal session/terminal exit only to cancel intentionally. Final completion
requires the conductor's explicit `complete_workflow`; an owner blocker requires
`owner_gate_workflow`, which suppresses pending wakes.
