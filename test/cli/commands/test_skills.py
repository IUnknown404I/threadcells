...
class TestSkillsRemoveCommand:
    ...
    def test_remove_refreshes_cao_managed_agent_prompt(self, runner, tmp_path, monkeypatch):
        ...
        (local_store / "custom-developer.md").write_text(
            "---\nname: custom-developer\ndescription: Developer\nprompt: Base prompt\n---\nBody\n",
            encoding="utf-8",
        )
        _create_skill(skill_store / "python-testing", "python-testing", "Pytest conventions")
        context_file = context_dir / "custom-developer.md"
        context_file.write_text("context", encoding="utf-8")
        agent_json = q_dir / "custom-developer.json"
        agent_json.write_text(
            json.dumps(
                {
                    "name": "custom-developer",
        ...
        refresh_agent_json_prompt(
            agent_json,
            AgentProfile(name="custom-developer", description="Developer", prompt="Base prompt"),
        )
